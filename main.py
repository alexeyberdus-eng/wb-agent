...
class WBClient:
    ...
    def reply_review(self, review_id, text):
        # WB API POST /api/v1/feedbacks/answer - flat format
        return self.post(self.FEEDBACK, "/api/v1/feedbacks/answer",
            {"id": review_id, "text": text})
...
SYSTEM_PROMPT = """Ты — умный агент управления магазином на Wildberries для бренда ESSOLA.
...
- При ответах на отзывы — следуй правилам WB: только о товаре, без рекламы и ссылок
...
async def generate_reply(req: GenerateReplyRequest):
    ...
    try:
        ...
        # Собираем полный контекст отзыва
        review_parts = []
        if req.text:
            review_parts.append(f"Текст: {req.text}")
        if req.pros:
            review_parts.append(f"Плюсы: {req.pros}")
        if req.cons:
            review_parts.append(f"Минусы: {req.cons}")
        if not review_parts:
            review_parts.append("(покупатель оставил только оценку без текста)")
        review_full = "\n".join(review_parts)

        prompt = f"""Ты — представитель службы заботы о клиентах бренда ESSOLA (натуральная косметика).
Напиши ответ на отзыв покупателя на Wildberries. Верни ТОЛЬКО текст ответа, без кавычек и пояснений.
...
- Отвечай по существу того что написал покупатель — если упомянул плюсы или минусы, отреагируй на них
- Если есть минусы или замечания — признай и объясни или пообещай учесть
- Иногда можно дать короткий уместный совет по использованию товара
- Без рекламы, без ссылок (правила WB)
- До 800 символов
- В конце подпись: Служба заботы о клиентах ESSOLA"""
...
async def generate_all_replies(req: GenerateAllRequest):
    """Генерация ответов на все отзывы через Claude (Haiku — дешевле)"""
    try:
        claude = anthropic.Anthropic(api_key=req.anthropic_key)
        results = []
        for rev in req.reviews:
            parts = []
            if rev.get("text"): parts.append(f"Текст: {rev['text']}")
            if rev.get("pros"): parts.append(f"Плюсы: {rev['pros']}")
            if rev.get("cons"): parts.append(f"Минусы: {rev['cons']}")
            if not parts: parts.append("(только оценка без текста)")
            review_full = "\n".join(parts)

            prompt = f"""Ты — представитель службы заботы о клиентах бренда ESSOLA (натуральная косметика).
Напиши ответ на отзыв покупателя на Wildberries. Верни ТОЛЬКО текст ответа, без кавычек.

Оценка: {rev.get('stars',5)} из 5 | Товар: {rev.get('product','—')} | Покупатель: {rev.get('userName','Покупатель')}
Отзыв:
{review_full}

Правила: пиши живо и по-человечески, отвечай на конкретные плюсы/минусы покупателя, иногда короткий совет по использованию, без рекламы и ссылок, до 800 символов.
В конце подпись: Служба заботы о клиентах ESSOLA"""
            r = claude.messages.create(
...
async def publish_reply(req: PublishReplyRequest):
    ...
    import logging
    logger = logging.getLogger("wb_publish")
    try:
        wb = WBClient(req.wb_token)
        # Логируем что отправляем
        payload = {"id": req.review_id, "text": req.text}
        logger.warning(f"PUBLISH PAYLOAD: {payload}")
        print(f"PUBLISH PAYLOAD: review_id={req.review_id!r} text_len={len(req.text)}")
        
        # Прямой запрос с полным логированием
        r = httpx.post(
            f"{wb.FEEDBACK}/api/v1/feedbacks/answer",
            headers=wb.h,
            json={"id": str(req.review_id), "text": str(req.text)},
            timeout=20
        )
        print(f"PUBLISH RESPONSE: status={r.status_code} body={r.text[:500]}")
        ...
        if r.status_code == 200:
            ...
            try:
                ...
                err_data = r.text
            raise HTTPException(r.status_code, detail=str(err_data))
